...
var log = tlog.NewTaggedLogger("WorkerManager", tlog.NewColor("38;5;111"))
...
func InitializeWorkers(loc *time.Location) *WorkerManager {
	wrk := &WorkerManager{
...
func (m *WorkerManager) AddWorkers() {
	for tag, worker := range m.Workers {
		...
		if err != nil {
			log.Errorf("Unable to register worker %s", tag)
			log.Fatal(err.Error())
		}
